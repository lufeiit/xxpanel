...
class StripeALLInOne {
    ...
    private function exchange($from, $to, $amount)
    {
        ...
        // 定义不同金额阈值的倍率
        $rateMultiplier = 1; // 默认倍率为1
        if ($amount < 10) { 
            $rateMultiplier = 1.5; 
        } elseif ($amount >= 10 && $amount < 20) { 
            $rateMultiplier = 1.1; 
        } elseif ($amount >= 20 && $amount < 30) { 
            $rateMultiplier = 1.08; 
        } elseif ($amount >= 30 && $amount < 50) { 
            $rateMultiplier = 1.05; 
        } elseif ($amount >= 50 && $amount < 230) { 
            $rateMultiplier = 1.02; 
        }
        ...
        try {
            ...
            if (isset($result['rates'][$to])) {
                $apiRate = $result['rates'][$to];
                // 在API汇率上乘以倍率
                return $apiRate * $rateMultiplier; // 将API汇率乘以相应倍率后返回
            }
            return false;
        } catch (\Exception $e) {
            info('货币兑换错误: '.$e->getMessage());
            return false;
        }
    }
    // 从user中获取email
